...
TEXT_MODELS=[
    "Qwen/Qwen2.5-VL-7B-Instruct:together",
    "Qwen/Qwen2.5-VL-14B-Instruct:together",
    "Qwen/Qwen2.5-VL-32B-Instruct:together",
    "mistralai/Mistral-7B-Instruct-v0.3:together",
    "mistralai/Mistral-8x7B-Instruct-v0.1:together",
...
def query_hf_api(payload:dict):
    ...
    if r.status_code !=200:
        try:
            j=r.json()
            msg=j.get("error",{}).get("massage") or str(j)
        except Exception:
...
def _extract_text(data)->str:
    msg=(data or {}).get("choices",[{}])[0].get("massage",{}) or {}
    return (msg.get("content") or "").strip()

def _run_models(models,massages,max_tokens=160,temperature=0.3):
    last_err=None
